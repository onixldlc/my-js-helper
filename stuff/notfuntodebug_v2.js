(async ()=>{class scrape_helper{
	constructor(initdata){
		this.mode = location.pathname.split("/").at(-1)

		this.key_dict = {
			"videos":{
				"initVideos": ["content", "contents"],
		...
		this.videos = grabValueFrom(initdata, this.key_dict[this.mode].initVideos)
		this.videosJson = []
		this.videosM3U = ""
		this.filename = ""
	...
	async getAllVids(){
		...
		while(x<100 && notFinish){
			...
			// debugger
			var videos = grabValueFrom(resp, this.key_dict[this.mode].videos)
